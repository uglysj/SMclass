...
-- 가입일, 1년 후 날짜를 출력하시오.
select sdate, sdate + 365 from students;
select sdate, add_months(sdate, 12) from students;
-- 현재일 기준으로 가입일이 6개월 이내의 회원만 출력하시오.
-- 11/1
...
select last_day(mdate) as md from member
order by md;
select substr(last_day(mdate), 1, 5) as md, count(*) from member
group by last_day(mdate)
order by last_day(mdate);

-- employees 테이블에서 부서별(department_id) 인월을 출력하시오.
...
rollback;
-- 제약조건을 확인
insert into emp4(employee_id, emp_name, salary, hire_date) select employee_id, emp_name, salary, hire_date from employees;
...
select * from emp4;
-- alter add: 테이블에 컬럼을 추가
alter table emp4
add(hire_date2 date);

desc emp4;

-- 컬럼 변경: 컬럼안에 데이터가 있다면 제약조건, 65 길이의 문자가 있을경우, 50으로 변경 안됨.
alter table emp4
...
select length(emp_name) from emp4
order by length(emp_name) desc;

-- 컬럼 타입 변경 -> 컬럼 안 데이터가 null 이면 가능
...
insert into bmember (id, pw, name, nicname, gender, email) values (
    'bbb', '1111', '유관순', '관순스', 'Female', 'bbb@bbb.com'
);
...
-- not null: null 허용하지 않음. 빈 공백은 가능함.
insert into bmember (id, pw, name, nicname, age, gender, email, bdate) values (
    'ddd', ' ', '강감찬', '감찬스', 20, 'Male', 'ddd@ddd.com', '2024/01/01'
);
-- primary key: 중복불가, null값 불가
insert into bmember (id, pw, name, nicname, age, gender, email, bdate) values (
    'eee', ' ', '김구', '구스', 20, 'Male', 'eee@eee.com', '2024/02/01'
);
commit;
select * from bmember;
...
insert into emp3 values(1, '홍', '01', '01');
insert into emp3 values(2, '유', '02', '02');
-- unique null 값은 허용
insert into emp3(ename, job, deptno) values('이', '03', '03');
-- unique 중복은 불가
insert into emp3 values(2, '강', '04', '04');
select * from emp3;
...
select * from member;
-- primary key 등록: 중복불가, null값 불가
insert into member values('fff', '1111', '홍길자', 'aaa@aaa.com', '123-456-7890', 'Female', 'golf', sysdate);
commit;
...
create table board(
    bno number(4) primary key, -- parimary key
...
insert into board values(board_seq.nextval, '제목1', '내용1', 'aaa', board_seq.currval, 0, 0, 0, sysdate, '');
insert into board values(board_seq.nextval, '제목2', '내용2', 'bbb', board_seq.currval, 0, 0, 0, sysdate, '');
insert into board values(board_seq.nextval, '제목5', '내용5', 'ddd', board_seq.currval, 0, 0, 0, sysdate, '');

commit;
select * from board; -- 개수 5개
select * from bmember; -- 개수 5개

-- 5개만 (5개 * 5개 = 25개)
select bno, btitle, bcontent,nicname, email, bgroup, bstep, bindent, bhit, bfile from board, bmember
